...
export const AdminRequestDetail: React.FC = () => {
  ...
  const loadDetails = async () => {
    ...
    try {
      ...
      if (data.request) {
        ...
        // Check if QR was already completed in subsequent checkpoints (steps 3-8)
        const isPastQrStep = [
  ...
  const currentStepIndex = React.useMemo(() => {
    if (!request) return 0;
    if ((request.workflowStatus as string) === 'crop_quality_check') {
      return 3; // Step 4: Weight & Quality
    }
  ...
  // Manual QR Override / Surpass Handler (Admin verifies using 6-digit PIN spoken/presented by farmer or scanned QR payload)
  const handleManualQrSurpass = (codeToVerify?: string) => {
    const code = (codeToVerify || manualCodeInput).trim();
    if (!code) {
      setStatusMessage({
        type: 'error',
        text: "Please enter the farmer's confidential 6-digit PIN (or scan QR pass) to verify arrival.",
      });
      return;
    }
    ...
    // Must match the secret 6-digit PIN or the scanned opaque QR ID (not the public token number)
    const isPinMatch = Boolean(pin && (lower === pin || lower.includes(pin)));
    const isQrIdMatch = Boolean(qrId && (lower === qrId || lower.includes(qrId)));

    if (isPinMatch || isQrIdMatch) {
      setQrVerificationStatus('verified');
      setQrVerifiedSuccess(true);
      setScannedQrCode(code);
      setStatusMessage({
        type: 'success',
        text: `FAIR VERIFICATION CONFIRMED: Farmer 6-digit PIN successfully authenticated for Token ${request?.token}. Gate pass arrival approved.`,
      });
    } else {
      setQrVerificationStatus('not_verified');
      setQrVerifiedSuccess(false);
      setStatusMessage({
        type: 'error',
        text: `VERIFICATION FAILED: "${code}" does not match this farmer's secret 6-digit PIN. Please verify the code with the farmer.`,
      });
    }
  };
  ...
  const handleStartProcurement = async () => {
    ...
    // Mandi Policy: Admin can only do procurement for the present day
    if (isFutureDate) {
      setStatusMessage({
        type: 'error',
        text: `RESTRICTED: Present-day procurement only! This appointment is booked for future date: ${bookingDay}. Mandi rules strictly forbid advance procurement before the scheduled day.`,
      });
      return;
    }
    ...
    if (ok) {
      setStatusMessage({
        type: 'success',
        text: 'Procurement started! Please scan the farmer\'s QR code or use the 6-digit PIN to authenticate arrival.',
      });
      await loadDetails();
    } else {
      setStatusMessage({
  ...
  const handleCompleteQualityCheckStep = async () => {
    if (!request) return;
    setAdvancing(true);
    setStatusMessage(null);
    ...
    if (ok) {
      setStatusMessage({
        type: 'success',
        text: `Quality check certified (${gradeLabel})! Grain custody procurement completed.`,
      });
      await loadDetails();
    } else {
        ...
        <div className="pt-5 border-t border-slate-100 bg-slate-50/70 -mx-6 -mb-6 p-6 rounded-b-2xl">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-slate-500 font-medium">
              Checkpoint #{currentStepIndex + 1} of {workflowSteps.length}
            </span>
          </div>
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-xs space-y-4">
              <div className="flex items-start gap-4">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${
                  ...
                  {isFutureDate ? <Clock className="w-6 h-6" /> : <Play className="w-6 h-6 ml-0.5" />}
                </div>
                <div>
                  <h5 className="text-sm font-bold text-slate-900">
                    {isFutureDate ? 'Scheduled for Future Date' : 'Start Physical Procurement Intake'}
                  </h5>
                  <p className="text-xs text-slate-600 mt-1">
                    {isFutureDate ? (
                      <span className="text-amber-800">
                        This appointment is booked for <strong>{bookingDay}</strong>. Mandi guidelines strictly prohibit conducting procurement for next days. Procurement can only be processed on the present day when the farmer physically arrives.
                      </span>
                    ) : (
                      'Click the button below when the farmer arrives at the Mandi gate. This starts the sequential procurement intake process, locks arrival into Counter #1, and enables the verification checkpoints.'
                    )}
                  </p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 pt-2">
                <button
                  type="button"
                  disabled={advancing || isFutureDate}
                  onClick={handleStartProcurement}
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 text-xs font-bold text-white hover:bg-emerald-500 shadow-xs transition disabled:opacity-50 disabled:cursor-not-allowed"
                >
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl border border-amber-300 bg-amber-50 hover:bg-amber-100 text-xs font-semibold text-amber-900 shadow-xs transition"
                >
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  <div className="relative flex-1 min-w-[220px]">
                    <input
                      ...
                      placeholder="Enter farmer's 6-digit PIN or scanned QR"
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-4 py-2 bg-amber-700 hover:bg-amber-800 text-white text-xs font-bold rounded-lg shadow-xs transition"
                  >
                    <KeyRound className="w-3.5 h-3.5" />
                    <span>Verify Farmer PIN</span>
                  </button>
                </div>
                <div className="flex flex-wrap items-center gap-3 text-[11px] text-slate-600 pt-1 border-t border-amber-200/60">
                  <span>Token: <strong className="font-mono text-slate-800">{request.token}</strong></span>
                  <span>•</span>
                  <span className="text-emerald-700 font-medium">6-Digit PIN: <strong>Confidential (Held by Farmer)</strong></span>
                  <span>•</span>
                  <span className="text-slate-500 font-medium">Opaque QR ID: <strong>Encrypted in QR matrix</strong></span>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-semibold text-slate-700">
                    {qrVerifiedSuccess && qrVerificationStatus === 'verified'
                      ...
                      : 'Awaiting QR Scan or Manual PIN Surpass'}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-xl border border-amber-300 bg-amber-50 hover:bg-amber-100 text-xs font-semibold text-amber-900 transition"
                  >
                    <PauseCircle className="w-4 h-4 text-amber-700" />
