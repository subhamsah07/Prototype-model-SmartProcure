...
class BookingService {
  ...
  async createBooking(params: CreateBookingParams): Promise<ProcurementBooking> {
    ...
    // 9. Generate unique random 6-character token, 6-digit verification PIN & opaque QR identifier
    ...
    if (insertError) {
      ...
      if (insertError.code === '23505') {
        ...
        const retryCode = generateVerificationCode();
        const retryQr = generateOpaqueQrPayload(tempId, retryToken, retryCode);
